...
class VMTranslator:
    ...
    def __init__(self, file_path):
        ...
        if(os.path.isfile(file_path)):
            ...
            self.rela_path=file_path
            self.asm_filename=self.vm_filename+".asm"
            self.asm_codes+=["@256","D=A","@SP","M=D"]
            self.codes+=["call Sys.init 0"]
            for file in os.listdir(file_path):
                name=os.path.basename(file)
                if name[-2:]=="vm":
                    self.files.append(file)
        for file in self.files:
                f_name=os.path.join(self.rela_path,file)
                with open(f_name,'r') as f:
                    self.vm_filename=os.path.basename(file)
                    for line in f:
                        _line=line.strip()
    ...
    def parse(self):
        ...
        for code in self.codes:
            ...
            if len(part)==1 and part[0]!="return": # arithmetic commands
                ...
                self.asm_codes+=self.C_return(part[0])
            elif part[0]=="call":
                self.asm_codes+=self.C_call(part[1:])
    
    def C_call(self,command):
        label=command[0]+"$ret."+str(self.call_num)
